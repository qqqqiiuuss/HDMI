...
class _cum_error_mixin:
    def __init__(self, min_steps: int=1, threshold: float=0.25, **kwargs):
        # Don't call super().__init__() here - this is handled by the actual termination class
        # Store mixin-specific parameters
...
class cum_body_pos_error(_cum_error_mixin, RobotTrackTermination):
    def __init__(self, body_names: str | List[str] = ".*", min_steps: int=1, threshold: float=0.25, **kwargs):
        # Call base termination __init__ first (this sets device, num_envs, etc.)
        BaseTermination.__init__(self, **kwargs)

        # Then initialize mixin (now device and num_envs are available)
...
class cum_joint_pos_error(_cum_error_mixin, RobotTrackTermination):
    def __init__(self, joint_names: str | List[str] = ".*", min_steps: int=1, threshold: float=0.25, **kwargs):
        BaseTermination.__init__(self, **kwargs)
        _cum_error_mixin.__init__(self, min_steps=min_steps, threshold=threshold)
...
class cum_lost_contact_steps(_cum_error_mixin, RobotObjectTrackTermination):
    def __init__(self, pos_thres: float=0.05, frc_thres: float=2.0, threshold: float=1.0, min_steps: int=1, **kwargs):
        BaseTermination.__init__(self, **kwargs)
        _cum_error_mixin.__init__(self, min_steps=min_steps, threshold=threshold)
